    ...
    <span class="flex col-span-4 mt-2 sm:mt-auto sm:justify-end">
        <nav aria-label="Table navigation">
            <ul class="inline-flex items-center">
                <li>
                    <button 
                        class="px-3 py-1 rounded-md rounded-l-lg focus:outline-none focus:shadow-outline-indigo"
                        aria-label="Previous"
                    ...
                    @if (is_array($element))
                        @foreach ($element as $page => $url)
                            @if ($page == $paginator->currentPage())
                                <li>
                                    <button
                                        class="px-3 py-1 text-white transition-colors duration-150 bg-indigo-600 border border-r-0 border-indigo-600 rounded-md focus:outline-none focus:shadow-outline-indigo">
                                        {{ $page }}
                                    </button>
                                </li>
                            @else
