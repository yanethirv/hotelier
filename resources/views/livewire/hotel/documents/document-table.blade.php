        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs mt-4">
                ...
                <table class="w-full whitespace-normal">
                    <thead>
                        <tr
                            ...
                            <th wire:click="sortBy('title')"  class="px-4 py-3">Title
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        ...
                        <tr class="text-gray-700 dark:text-gray-400">
                            ...
                            <td class="px-4 py-3 text-sm">
                                <div class="flex m-2">
                                    <a href="{{ asset('storage/'.$document->attachment) }}" target="_blank" rel="noopener noreferrer" class="text-xs mr-2 rounded-l-none hover:scale-110 focus:outline-none flex justify-center px-4 py-2 rounded font-semibold cursor-pointer 
                                            hover:bg-teal-600 hover:text-teal-100 bg-teal-100 text-teal-600 border duration-200 ease-in-out border-none transition">
                                        <div class="flex leading-5">{{ __("View Attachment") }}</div>
                                    </a>
                                </div>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <a wire:click="showModal({{$document->id}})" href="javascript:void(0)"
                                    ...
                                    <a wire:click="deleteConfirm({{$document->id}})" href="javascript:void(0)"
            ...
            <div
                ...
                <span class="flex items-center col-span-3">
                    Showing {{ $documents->firstItem() }} to {{ $documents->lastItem() }} out of {{ $documents->total() }}
                </span>
            ...
            window.addEventListener('swal:confirm', event => {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.livewire.emit('deleteDocument', event.detail.id);
                        Toast.fire({
                        icon: 'success',
