        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs mt-4">
                ...
                <table class="w-full whitespace-normal">
                    <thead>
                        <tr
                            ...
                            <th class="px-4 py-3">Activation Service</th>
                            <th class="px-4 py-3">Status</th>
                            <th class="px-4 py-3">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        @foreach ($activationServicesTransactions as $activationServiceTransaction)
                        <tr class="text-gray-700 dark:text-gray-400">
                            <td class="px-4 py-3 text-sm">
                                {{$activationServiceTransaction->id}}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{$activationServiceTransaction->user->name}}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{$activationServiceTransaction->activationService->name}}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                <span
                                    class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">
                                    {{$activationServiceTransaction->requestStatus->name}}
                                </span>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <a wire:click="showModal({{$activationServiceTransaction->id}})" href="javascript:void(0)"
            ...
            <div
                ...
                <span class="flex items-center col-span-3">
                    Showing {{ $activationServicesTransactions->firstItem() }} to {{ $activationServicesTransactions->lastItem() }} out of {{ $activationServicesTransactions->total() }}
                </span>
                <span class="col-span-2"></span>
                <!-- Pagination -->
                <span class="flex col-span-4 mt-2 sm:mt-auto sm:justify-end">
                    {{ $activationServicesTransactions->links() }}
                </span>
            </div>
        </div>
    </div>
