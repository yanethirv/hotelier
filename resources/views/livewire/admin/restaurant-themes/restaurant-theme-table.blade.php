        ...
        <div class="bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:bg-gray-800 py-3 items-center justify-between">
            ...
            <div class="flex text-gray-500 pt-4">
                <button wire:click="showModal" type="button"
                    ...
                    <span>Add Restaurant Theme</span>
                </button>
            </div>
        </div>
        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs mt-4">
            <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-normal">
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        @foreach ($restaurantThemes as $restaurantTheme)
                        <tr class="text-gray-700 dark:text-gray-400">
                            <td class="px-4 py-3 text-sm">
                                {{$restaurantTheme->id}}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{$restaurantTheme->name}}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                {{$restaurantTheme->description}}
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <a wire:click="showModal({{$restaurantTheme->id}})" href="javascript:void(0)"
                                    ...
                                    <a wire:click="deleteConfirm({{$restaurantTheme->id}})" href="javascript:void(0)"
            ...
            <div
                ...
                <span class="flex items-center col-span-3">
                    Showing {{ $restaurantThemes->firstItem() }} to {{ $restaurantThemes->lastItem() }} out of {{ $restaurantThemes->total() }}
                </span>
                <span class="col-span-2"></span>
                <!-- Pagination -->
                <span class="flex col-span-4 mt-2 sm:mt-auto sm:justify-end">
                    {{ $restaurantThemes->links() }}
                </span>
            </div>
        </div>
    </div>
            ...
            window.addEventListener('swal:confirm', event => {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.livewire.emit('deleteRestaurantTheme', event.detail.id);
                        Toast.fire({
                        icon: 'success',
                        title: 'Restaurant Theme has been deleted.',
                        })
                    }
                });
