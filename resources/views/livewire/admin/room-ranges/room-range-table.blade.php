        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs mt-4">
            <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-normal">
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        @foreach ($roomRanges as $roomRange)
                        <tr class="text-gray-700 dark:text-gray-400">
                            ...
                            <td class="px-4 py-3 text-sm">
                                {{$roomRange->description}}
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <a wire:click="showModal({{$roomRange->id}})" href="javascript:void(0)"
                                    ...
                                    <a wire:click="deleteConfirm({{$roomRange->id}})" href="javascript:void(0)"
            ...
            <div
                ...
                <span class="flex items-center col-span-3">
                    Showing {{ $roomRanges->firstItem() }} to {{ $roomRanges->lastItem() }} out of {{ $roomRanges->total() }}
                </span>
            ...
            window.addEventListener('swal:confirm', event => {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.livewire.emit('deleteRoomRange', event.detail.id);
                        Toast.fire({
                        icon: 'success',
                        title: 'Room Range has been deleted.',
                        })
                    }
                });
